...
#data = exportData(wantedCollection="matches")
data = getDataWithQuery("matches", ["gameType", "image"])
'''
data format
{'test': {241231:{'winner':2, 'loser':2, 'timestamp':2, 'prompt':2}, 513123:{}, ...}}
'''
print(data.keys())
imageMatches = pd.DataFrame.from_dict(data, orient="index").sort_values(ascending=True, by=["timestamp"])
...
matchRates = predict_win_rate(compute_elo(matches))
statsData = {'stats':{'image':{}}}

#print(matchRates)
for i in matchRates.columns:
    statsData['stats']['image'][i] = {}
    for j in matchRates.columns:
        if(i != j):
            statsData['stats']['image'][i][j] = matchRates[i][j]
    statsData['stats']['image'][i]['elo'] = bootstrap_lu_median.get(bootstrap_lu_median['model'] == i)['rating'].values[0]
